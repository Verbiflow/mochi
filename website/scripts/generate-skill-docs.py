...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Verbiflow/mochi/blob/main/{source_dir}/{meta['rel_path']}"
...
def prune_stale_generated_pages(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> int:
    """Remove generated per-skill pages whose source SKILL.md no longer exists."""
    expected = {page_output_path(meta).resolve() for meta, _ in entries}
    removed = 0
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Mochi"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Mochi ships with a built-in skill library copied into `~/.hermes/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Mochi also syncs bundled skills on `hermes update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hermes/skills/` tree, it is still shipped with Mochi; restore it with `hermes skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with Mochi — install via hermes skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with Mochi under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
...
def write_sidebar(entries):
    ...
    # Individual skill pages (including the formerly hand-written
    # google-workspace guide) are still reachable at their URLs and are
...
def main():
    ...
    pruned = prune_stale_generated_pages(entries)
    if pruned:
        print(f"Pruned {pruned} stale generated skill pages")
