...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # /model work directly without /mochi prefix). A single regex
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/mochi$")

            @self._app.command(_slash_pattern)
            async def handle_mochi_command(ack, command):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Mochi handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Mochi session — matching the per-thread behavior channels already
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The parent ``/mochi <subcommand> [args]`` form is the generated
        manifest path. Users can also ask free-form questions through the
        parent slash (``/mochi
        ...
        if slash_name in {"mochi", ""}:
            # Parent /mochi <subcommand> [args] routing + free-form questions.
        ...
        # "Running /cmd…" ack shown by handle_mochi_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/mochi <question>" must produce public replies so
    ...
    def _has_active_session_for_thread(
        ...
        try:
            ...
            source = SessionSource(
                ...
                user_id=user_id,
                thread_id=thread_ts,
                guild_id=self._channel_team.get(channel_id),
            )
