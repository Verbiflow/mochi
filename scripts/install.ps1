...
$RepoUrlSsh = if ($env:MOCHI_REPO_URL_SSH) { $env:MOCHI_REPO_URL_SSH } else { "git@github.com:Verbiflow/mochi.git" }
$RepoUrlHttps = if ($env:MOCHI_REPO_URL) { $env:MOCHI_REPO_URL } else { "https://github.com/Verbiflow/mochi.git" }
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Install-Repository {
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/Verbiflow/mochi/archive/refs/heads/$Branch.zip"
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Verbiflow/mochi/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
